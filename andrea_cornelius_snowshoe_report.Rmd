...
**Figure 1: The snowshoe hare population was largest in 1999, at a member count of 126.  declined with peaks in 2005, 2008, and 2011. Minimum in 2010 with a count of 2, varying between. Not much data taken in 2002 or 2009 (not many visits to traps), hence no counts there**
...
```
```{r}
female_sample <- hare_subset_2 %>%
  filter(sex == "female") %>%
  pull(weight)

male_sample <- hare_subset_2 %>%
  filter(sex == "male") %>%
  pull(weight)

weight_t <- t.test(female_sample, male_sample)
```
